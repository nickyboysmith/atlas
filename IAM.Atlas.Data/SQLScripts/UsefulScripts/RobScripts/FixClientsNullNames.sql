...
--SELECT FP.*, T.*
UPDATE FP
SET FP.AlternateId = T.Id
FROM @FakePeople FP 
INNER JOIN (SELECT C.Id, ROW_NUMBER() OVER(ORDER BY C.Id ASC) AS RowNum
			FROM Client C
			INNER JOIN Gender G ON G.Id = C.GenderId
			WHERE LEN(ISNULL(C.FirstName,'')) <= 0 AND LEN(ISNULL(C.Surname,'')) <= 0 
			AND G.Name = 'Unknown'
			) T ON T.RowNum = FP.Id
WHERE FP.AlternateId IS NULL;
...
UPDATE C
SET C.Title = ''
, C.FirstName = FP.FirstName
, C.Surname = FP.Surname
, C.DisplayName = FP.FirstName + ' ' + FP.Surname
FROM @FakePeople FP
INNER JOIN Client C ON FP.AlternateId = C.Id
INNER JOIN Gender G ON G.Id = C.GenderId
WHERE LEN(ISNULL(C.FirstName,'')) <= 0 AND LEN(ISNULL(C.Surname,'')) <= 0
--WHERE LEN(ISNULL(T.Surname,'')) <= 0
--AND G.Name = 'Female'
--AND G.Name = 'Male'
AND G.Name = 'Unknown'
...
UPDATE Client
SET FirstName = 'A'
WHERE FirstName IS NULL AND Surname IS NOT NULL


UPDATE Client
SET Surname = 'Zed'
WHERE FirstName IS NOT NULL AND Surname IS NULL


UPDATE Client
SET DisplayName = LTRIM(RTRIM(ISNULL(Title,'') + ' ' + FirstName + ' ' + Surname))
WHERE LEN(ISNULL(DisplayName,'')) <= 0


UPDATE Client
SET DisplayName = LTRIM(RTRIM(DisplayName))
WHERE LEN(ISNULL(DisplayName,'')) != LEN(LTRIM(RTRIM(DisplayName)))
