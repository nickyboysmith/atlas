...
DECLARE @ScriptName VARCHAR(100) = 'SP037_08.01_AmendInsertTriggerOnVenue.sql';
...
CREATE TRIGGER [dbo].[TRG_VenueToInsertInToTrainerVenue_Insert] ON [dbo].[Venue] AFTER INSERT
AS
BEGIN
	...
	BEGIN                 
		...
		-------------------------------------------------------------------------------------------
			...
			IF (@rowCount > 0 AND OBJECT_ID('tempdb..#Venue', 'U') IS NOT NULL)
			BEGIN
