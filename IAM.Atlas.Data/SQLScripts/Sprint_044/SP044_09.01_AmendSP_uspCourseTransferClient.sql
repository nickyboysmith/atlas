/*
	SCRIPT: Amend uspCourseTransferClient
	Author: Paul Tuck
	Created: 27/09/2018
*/

DECLARE @ScriptName VARCHAR(100) = 'SP044_09.01_AmendSP_uspCourseTransferClient.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend uspCourseTransferClient';
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					-- if @dorsAttendanceRef is NULL get it from the ClientDORSData table
					IF (@dorsAttendanceRef IS NULL)
					BEGIN
						SELECT 
							@dorsAttendanceRef = cdd.dorsAttendanceRef,
							@DORSAttendanceStateIdentifier = das.DORSAttendanceStateIdentifier
						FROM ClientDORSData cdd
						INNER JOIN dorsattendancestate das ON das.id = cdd.DORSAttendanceStateId
						INNER JOIN courseclient cc ON cc.clientid = cdd.clientid
						INNER JOIN course ON course.id = cc.courseid
						INNER JOIN dorsschemecoursetype dsct ON dsct.coursetypeid = course.coursetypeid
						INNER JOIN dorsScheme ds ON ds.id = dsct.dorsschemeid
												AND	ds.id = cdd.dorsschemeid
						WHERE  cdd.clientid = @fromClientId
							AND	course.id = @fromCourseId
						;
					END
