...
DECLARE @ScriptName VARCHAR(100) = 'SP044_11.01_AmendSP_uspGetReportData.sql';
...
GO
	...
	*/
	...
	BEGIN
		...
		SET @SQL = 'SELECT * FROM ('
		SET @SQL = @SQL + 'SELECT TOP ' + CAST(@MaxReportRows AS VARCHAR) + ' ' + CAST(@ReportId AS VARCHAR) + ' AS ReportId '
		...
		BEGIN
			...
			SELECT @SQL = @SQL + ', '
						+ (CASE WHEN RC.ColumnDataType = 'DateTime'
									THEN 'CAST(C' + CAST(RC.ColumnDisplayOrder AS VARCHAR) + ' AS DATETIME)'
								ELSE 'C' + CAST(RC.ColumnDisplayOrder AS VARCHAR)
								END)    ---RC.ColumnName
			FROM vwReportColumn RC
			WHERE RC.[ReportId] = @ReportId
