...
DECLARE @ScriptName VARCHAR(100) = 'SP044_01.01_Alter_SP_uspDashboardDataRefresh_Client.sql';
	...
	BEGIN
		...
		BEGIN TRY 
			...
			SELECT 
				...
				, ISNULL(CWMDDS.TotalCreatedThisWeek, 0) AS ClientsWithMissingReferringAuthorityCreatedThisWeek
				, ISNULL(CWMDDS.TotalCreatedThisMonth, 0) AS ClientsWithMissingReferringAuthorityCreatedThisMonth
				, ISNULL(CWMDDS.TotalCreatedLastMonth, 0) AS ClientsWithMissingReferringAuthorityCreatedLastMonth
			...
			LEFT JOIN vwClientsWithMissingDORSDataSummary CWMDDS ON CWMDDS.OrganisationId = O.Id
			GROUP BY O.Id
					, O.Name
