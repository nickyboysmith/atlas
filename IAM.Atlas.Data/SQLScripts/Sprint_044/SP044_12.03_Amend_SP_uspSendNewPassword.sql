/*
	...
	Created: 02/10/2017
*/
DECLARE @ScriptName VARCHAR(100) = 'SP044_12.03_Amend_SP_uspSendNewPassword.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend SP uspSendNewPassword';
	...
	BEGIN
		...
		BEGIN
			DECLARE @LastLetterOfCurrentMonth CHAR(1)
				  ...
				  , @organisationId INT
				  , @Name VARCHAR(320)
				  , @userIsSysAdmin BIT = 'False'
				  , @userIsClient BIT = 'False'
				  , @LoginEmailSendingEnabled BIT = 'False'
			...
			IF (EXISTS(SELECT * FROM [dbo].[SystemAdminUser] WHERE UserId = @UserId))
			BEGIN
				SET @userIsSysAdmin = 'True';

				SELECT
					@fromName = SC.AtlasSystemFromName	
					, @fromEmailAddresses = SC.AtlasSystemFromEmail	
			...
			BEGIN
				...
				IF EXISTS (SELECT c.id 
						   ...
						   WHERE u.Id = @UserId)
				BEGIN
					--User is a Client User
					SELECT TOP 1
						@emailSubject = ISNULL(OSC.ClientApplicationDescription, @emailSubject)
						, @organisationId = co.OrganisationId
						, @toEmailAddresses = ISNULL(E.[Address], @toEmailAddresses)
						, @Name = ISNULL(C.DisplayName, @Name)
					FROM [User] u
					INNER JOIN Client c ON u.id = c.UserId
				...
				BEGIN
					...
					-- if @organisationId is still NULL then check the trainerOrganisation table...
					IF @organisationId IS NULL
					BEGIN
						select @organisationId = torg.organisationId from trainer t
						INNER JOIN trainerorganisation torg on t.id = torg.trainerid
			...
			SELECT @LoginEmailSendingEnabled = LoginEmailSendingEnabled FROM [OrganisationSystemConfiguration] WHERE OrganisationId = @organisationId;
			
			IF EXISTS(SELECT Id FROM Client WHERE UserId = @UserId)
			BEGIN
				SET @userIsClient = 'True';
			END


			IF ((@userIsClient = 'False') OR (@userIsClient = 'True' AND @LoginEmailSendingEnabled = 'True'))
			BEGIN
