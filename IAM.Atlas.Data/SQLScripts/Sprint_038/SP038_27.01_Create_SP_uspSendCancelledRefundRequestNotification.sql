/*
	SCRIPT: Create uspSendCancelledRefundRequestNotification
	Author: Dan Hough
	Created: 06/06/2017
*/

DECLARE @ScriptName VARCHAR(100) = 'SP038_27.01_Create_SP_uspSendCancelledRefundRequestNotification.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Create uspSendCancelledRefundRequestNotification';
...
*/		
IF OBJECT_ID('dbo.uspSendCancelledRefundRequestNotification', 'P') IS NOT NULL
BEGIN
	DROP PROCEDURE dbo.uspSendCancelledRefundRequestNotification;
END		
GO
	/*
		Create uspSendCancelledRefundRequestNotification
	*/

	CREATE PROCEDURE dbo.uspSendCancelledRefundRequestNotification (@refundRequestId INT, @cancellationReason VARCHAR(100))
	AS
	BEGIN
		DECLARE @organisationDisplayNameTag CHAR(29) = '<!Organisation Display Name!>'
				...
				, @cancellationReasonTag CHAR(22) = '<!CancellationReason!>'
				, @canRefRequestUser CHAR(18) = 'CanRefRequest-User'
				, @canRefRequestAdmin CHAR(19) = 'CanRefRequest-Admin'
		...
		INNER JOIN dbo.OrganisationEmailTemplateMessage OETM ON VWRR.RefundRequestOrganisationId = OETM.OrganisationId
																AND OETM.Code = @canRefRequestUser
		INNER JOIN (SELECT OrganisationId
					...
					WHERE Code = @canRefRequestAdmin) A ON OETM.OrganisationId = A.OrganisationId
		...
		SELECT @userEmailContent = REPLACE(@userEmailContent, @cancellationReasonTag, @cancellationReason);
		...
		SELECT @adminEmailContent = REPLACE(@adminEmailContent, @cancellationReasonTag, @cancellationReason);
		...
		BEGIN
			UPDATE dbo.CancelledRefundRequest
			SET DateCancellationNotificationSent = GETDATE()
