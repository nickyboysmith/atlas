...
*/
IF OBJECT_ID('dbo.vwCourseListPast', 'V') IS NOT NULL
BEGIN
	DROP VIEW dbo.vwCourseListPast;
END		
GO

/*
	Create vwCourseListPast
*/
CREATE VIEW vwCourseListPast
AS
	SELECT
	...
	WHERE COD.DateStart BETWEEN DATEADD(MONTH, -3, GETDATE()) AND GETDATE() 
	GROUP BY
		OCO.OrganisationId
		, O.[Name]
		, CO.Id
