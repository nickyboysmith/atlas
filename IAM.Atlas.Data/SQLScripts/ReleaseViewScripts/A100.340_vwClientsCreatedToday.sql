...
IF OBJECT_ID('dbo.vwClientsCreatedToday', 'V') IS NOT NULL
BEGIN
	DROP VIEW dbo.vwClientsCreatedToday;
END		
GO
		

/*
	Create View vwClientsCreatedToday
*/
CREATE VIEW dbo.vwClientsCreatedToday
AS
	SELECT
	...
	WHERE CAST(C.[DateCreated] AS DATE) = CAST((Getdate() - 0) AS DATE)
	;
GO
