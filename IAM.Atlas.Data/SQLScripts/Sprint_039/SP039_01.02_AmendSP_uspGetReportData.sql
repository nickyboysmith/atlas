...
DECLARE @ScriptName VARCHAR(100) = 'SP039_01.02_AmendSP_uspGetReportData.sql';
...
GO
	...
	*/
	...
	BEGIN
		...
		SET @SQL = @SQL + ', 999 AS RowOrder'
		----------------------------------------------------------------------------------------------------------------------------------------*/

		--Now the ORDER BY
		IF EXISTS (SELECT * FROM vwReportColumn RC WHERE RC.[ReportId] = @ReportId AND ISNULL(RC.ColumnSortOrder,-1) >= 0)
		BEGIN
			SET @SQL = @SQL + CHAR(13) + CHAR(10) + ' ORDER BY RowOrder '
			SELECT @SQL = @SQL 
					--+ (CASE WHEN @SQL LIKE '%ORDER BY RowOrder ' THEN '' ELSE ', ' END)
					+ ', C' + CAST(RC.ColumnDisplayOrder AS VARCHAR)    ---RC.ColumnName
			FROM vwReportColumn RC
			WHERE RC.[ReportId] = @ReportId
