/*
	SCRIPT: Alter birthdate function to calculate century from a UK Licence number
	Author: Nick Smith
	Created: 21/07/2017
*/

DECLARE @ScriptName VARCHAR(100) = 'SP041_01.01_AlterFunctionudfReturnBirthdateFromUkLicenceNumber.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Alter birthdate function to calculate century from a UK Licence number';
	...
	*/
	...
	BEGIN
		...
		BEGIN
			...
			-- If dob year from licence is less than todays year then it 2000, else 1900
			SELECT @currentyear = RIGHT(Year( getdate() ) ,2)
			
			IF (CAST(@year AS INT) <= @currentyear)
			BEGIN
				SELECT @century = '20';
			END
			ELSE
			...
			SELECT @dob = CAST(@century + @year + @month + @day AS DATE);
		END
		ELSE
		BEGIN
			SELECT @dob = NULL;
