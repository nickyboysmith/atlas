/*
 * SCRIPT: Alter uspSendNewUserEmail
 * Author: Paul Tuck
 * Created: 27/07/2017
 */

DECLARE @ScriptName VARCHAR(100) = 'SP041_12.01_Amend_SP_uspSendNewUserEmail.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Alter uspSendNewUserEmail';
...
GO
	...
	*/
	CREATE PROCEDURE [dbo].[uspSendNewUserEmail]
	(
		@clientId INT = NULL,
		@userId INT = NULL
	)
	...
	BEGIN
		...
		*/
		...
		-- is the sending login emails feature enabled for this organisation?
		Select @LoginEmailSendingEnabled = LoginEmailSendingEnabled FROM [OrganisationSystemConfiguration] WHERE OrganisationId = @orgId;

		IF (@loginNotified != 'True' AND @LoginEmailSendingEnabled = 'True')
		BEGIN
