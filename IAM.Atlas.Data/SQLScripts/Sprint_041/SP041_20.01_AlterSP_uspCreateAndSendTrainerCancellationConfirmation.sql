/*
	SCRIPT: Alter Stored procedure uspCreateAndSendTrainerCancellationConfirmation
	Author: Dan Hough
	Created: 28/07/2017

*/

DECLARE @ScriptName VARCHAR(100) = 'SP041_20.01_AlterSP_uspCreateAndSendTrainerCancellationConfirmation';
DECLARE @ScriptComments VARCHAR(800) = 'Alter Stored procedure uspCreateAndSendTrainerCancellationConfirmation';
...
*/		
IF OBJECT_ID('dbo.uspCreateAndSendTrainerCancellationConfirmation', 'P') IS NOT NULL
BEGIN
	DROP PROCEDURE dbo.[uspCreateAndSendTrainerCancellationConfirmation];
END		
GO
	/*
		Create [uspCreateAndSendTrainerCancellationConfirmation]
	*/
	
	CREATE PROCEDURE [dbo].[uspCreateAndSendTrainerCancellationConfirmation](@courseId INT, @trainerId INT)
	AS
	BEGIN
		...
		DECLARE @trainerDisplayNameTag CHAR(22) = '<!TrainerDisplayName!>'
				...
				, @orgId INT
				, @systemTrappedFeatureName CHAR(47) = 'uspCreateAndSendTrainerCancellationConfirmation'
				, @systemTrappedMessage VARCHAR(8000)
				, @templateCode CHAR(15) = 'TrainerCancConf'
