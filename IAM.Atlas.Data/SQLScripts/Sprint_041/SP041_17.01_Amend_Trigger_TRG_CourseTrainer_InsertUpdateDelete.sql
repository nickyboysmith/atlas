...
DECLARE @ScriptName VARCHAR(100) = 'SP041_17.01_Amend_Trigger_TRG_CourseTrainer_InsertUpdateDelete.sql';
...
GO 
	...
	BEGIN
		...
		BEGIN --START PROCESS
			...
			------------------------------------------------------------------------------------------------
			DECLARE @deletedCourseTrainerId INT;
			DECLARE @insertedCourseTrainerId INT;

			SELECT @deletedCourseTrainerId = d.Id FROM Deleted d;
			SELECT @insertedCourseTrainerId = i.Id FROM Inserted i;
			...
			IF (@deletedCourseTrainerId IS NULL)
			BEGIN
				EXEC uspCreateAndSendTrainerBookingConfirmation @insertedCourseTrainerId;
			END

			--send trainer cancellation conf if it's an insert
			IF (@insertedCourseTrainerId IS NULL)
			BEGIN
				EXEC uspCreateAndSendTrainerCancellationConfirmation @deletedCourseTrainerId
			END
		END --END PROCESS
	END
