/*
	SCRIPT: Amend trigger CourseClientTransferRequest
	Author: Dan Hough
	Created: 09/08/2017
*/

DECLARE @ScriptName VARCHAR(100) = 'SP041_32.01_Amend_Trigger_CourseClientTransferRequest.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend Trigger CourseClientTransferRequest';
...
CREATE TRIGGER [TRG_CourseClientTransferRequest_Insert] ON [dbo].[CourseClientTransferRequest] AFTER INSERT
AS
BEGIN
	...
	BEGIN --START PROCESS
		...
		DECLARE @allowAutoTransferEmailsToClient BIT;

		SELECT @allowAutoTransferEmailsToClient = OSC.AllowAutoCourseTransferEmailsToClient 
		FROM Inserted i
		INNER JOIN dbo.Course C ON i.TransferFromCourseId = C.Id
		INNER JOIN dbo.OrganisationSelfConfiguration OSC ON C.OrganisationId = OSC.OrganisationId;

		IF (@allowAutoTransferEmailsToClient = 'True')
		BEGIN
			...
			INNER JOIN dbo.OrganisationDisplay od ON ctFrom.OrganisationId = od.OrganisationId
			...
			SELECT @clientEmail = EmailAddress
				 , @clientName = CASE WHEN DisplayName IS NULL OR LEN(DisplayName) <= 0
										THEN 
