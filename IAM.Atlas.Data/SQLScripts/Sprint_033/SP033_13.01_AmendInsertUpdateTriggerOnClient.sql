/*
	SCRIPT: Amend Update and Insert trigger on Client
	Author: Dan Hough
	Created: 09/02/2017
*/

DECLARE @ScriptName VARCHAR(100) = 'SP033_13.01_AmendInsertUpdateTriggerOnClient.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend Update and Insert trigger on Client';
...
IF OBJECT_ID('dbo.TRG_Client_INSERTUPDATE', 'TR') IS NOT NULL
	...
	END
GO
	CREATE TRIGGER TRG_Client_INSERTUPDATE ON dbo.Client AFTER UPDATE, INSERT
	AS
	BEGIN
