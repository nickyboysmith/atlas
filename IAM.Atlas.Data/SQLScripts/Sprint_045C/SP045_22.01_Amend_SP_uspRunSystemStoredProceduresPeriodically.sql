...
DECLARE @ScriptName VARCHAR(100) = 'SP045_22.01_Amend_SP_uspRunSystemStoredProceduresPeriodically.sql';
	...
	*/
	...
	DECLARE @ProcedureName VARCHAR(200) = 'uspRunSystemStoredProceduresPeriodically'
		...
		BEGIN TRY 
			...
			CREATE TABLE #SPToProcess(
				PeriodicalSPJobId INT
				, SPName VARCHAR(400)
				, [DueDate] [date]
				, [DateLastRun] [datetime]
				, [RunEveryAfterMinutes] [int]
				, [RunAfterDays] [int] 
				, [RunAfterMonths] [int]
				...
				, [RunOnlyOnWeekends] [bit]
				, [RunOnlyOnWeekdays] [bit]
				, [RunOnOrAfterTime] [time](7)
				, [RunOnOrBeforeTime] [time](7)
				, [LastDateTimeStarted] [datetime]
				);

			INSERT INTO #SPToProcess
			SELECT Id AS PeriodicalSPJobId
					, StoredProcedureName AS SPName
					, DueDate
					, DateLastRun
			...
			BEGIN
				...
				BEGIN
					...
					IF (DATEDIFF(HOUR, @lastDateTimeStarted, GETDATE()) >= 1 
						AND @lastDateTimeStarted > @dateLastRun
						)
					BEGIN
					...
					SET @lastDateTimeStarted = ISNULL(@lastDateTimeStarted, CAST('01/01/01' AS DATE));
					SET @dateLastRun = ISNULL(@dateLastRun, CAST('01/01/02' AS DATE));
									...
									BEGIN 									
										...
										BEGIN
											...
											BEGIN
												...
												SET LastDateTimeStarted = GETDATE()
