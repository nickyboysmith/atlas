/*
	SCRIPT: Amend CourseDate Triggers
	Author: Robert Newnham
	Created: 17/01/2016
*/

DECLARE @ScriptName VARCHAR(100) = 'SP032_13.01_AmendCourseDateTriggers.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend Course Date Triggers';

EXEC dbo.uspScriptStarted @ScriptName, @ScriptComments; /*LOG SCRIPT STARTED*/
GO 

IF OBJECT_ID('TRG_CourseDate_INSERT', 'TR') IS NOT NULL
	BEGIN
		DROP TRIGGER dbo.TRG_CourseDate_INSERT;
	END
GO
IF OBJECT_ID('TRG_CourseDate_DELETE', 'TR') IS NOT NULL
	BEGIN
		DROP TRIGGER dbo.TRG_CourseDate_DELETE;
	END
GO
IF OBJECT_ID('TRG_CourseDate_UPDATE', 'TR') IS NOT NULL
	BEGIN
		DROP TRIGGER dbo.TRG_CourseDate_UPDATE;
	END
GO
...
GO
	...
	BEGIN
		...
		BEGIN
			...
			-----------------------------------------------------------------------------------------------------------------------
			select CAST('True' AS BIT),  CAST(CAST('True' AS BIT) AS VARCHAR)
