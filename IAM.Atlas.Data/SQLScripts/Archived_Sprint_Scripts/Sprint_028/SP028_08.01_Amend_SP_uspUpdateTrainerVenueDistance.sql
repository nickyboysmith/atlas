/*
	SCRIPT: Amend uspUpdateTrainerVenueDistance
	Author: Dan Hough
	Created: 21/10/2016

	Changed va.Id to va.VenueId where it's setting 
	the @venuePostCode parameter
*/

DECLARE @ScriptName VARCHAR(100) = 'SP028_08.01_Amend_SP_uspUpdateTrainerVenueDistance.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Create uspUpdateTrainerVenueDistance';
...
GO
	...
	*/
	...
	BEGIN
		...
		SELECT @trainerVenueId = id FROM dbo.TrainerVenue WHERE TrainerId = @TrainerId AND VenueId = @VenueId
