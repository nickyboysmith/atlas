...
DECLARE @ScriptName VARCHAR(100) = 'SP029_22.01_Amend_uspArchiveEmails.sql';
...
BEGIN
	...
	IF (@uspArchiveEmailLastRunLog IS NULL)
	  ...
	  WHERE SE.ScheduledEmailStateId = 2
		AND GETDATE() > DATEADD(day, OAC.ArchiveEmailsAfterDays, DateScheduledEmailStateUpdated)
		OR (O.Id IS NULL AND GETDATE() > DATEADD(day, @defaultArchiveDays, DateScheduledEmailStateUpdated)) -- when not linked to an organisation, then use default on SystemControl
