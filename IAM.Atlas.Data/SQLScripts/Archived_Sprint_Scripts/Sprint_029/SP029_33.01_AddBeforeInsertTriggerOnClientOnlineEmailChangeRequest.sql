/*
	SCRIPT: Add before insert trigger on the ClientOnlineEmailChangeRequest table
	Author: Dan Hough
	Created: 24/11/2016
*/

DECLARE @ScriptName VARCHAR(100) = 'SP029_33.01_AddBeforeInsertTriggerOnClientOnlineEmailChangeRequest.sql';
...
BEGIN
	...
	SELECT @currentYear					= CAST(DATEPART(YEAR, GETDATE()) AS CHAR) 
		 ...
		 , @monthNumber					= CASE WHEN 
											DATEPART(MONTH, GETDATE()) < 10
										  THEN
											 '0' + CAST(DATEPART(MONTH, GETDATE()) AS CHAR)
										  ELSE
		...
		, @rowCount						= COUNT(Id)
										  FROM dbo.ClientOnlineEmailChangeRequest
										  WHERE MONTH(DateRequested) = MONTH(CURRENT_TIMESTAMP) -- gets the count of rows on dbo.ClientOnlineEmailChangeRequest
