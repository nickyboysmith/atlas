...
DECLARE @ScriptName VARCHAR(100) = 'SP029_48.01_Create_uspCourseTransferClient.sql';
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO dbo.Note (Note
								, DateCreated
								, CreatedByUserId)
						VALUES ('Transferred into Course Id: ' + CAST(@toCourseId AS VARCHAR) + ' replacing Client: ' + @fromClientName + ' on Course Id: ' + CAST(@toCourseId AS VARCHAR)
								, GETDATE()
								, @requestedByUserId);
		...
		INSERT INTO dbo.CourseNote(CourseId
							...
							VALUES (@fromCourseId
									, 1 --General 
									, 'Client Name: ' + @fromClientName + ', Id: ' + CAST(@fromClientId AS VARCHAR) + ' - transferred out of this course on to Course Id: ' + CAST(@toCourseId AS VARCHAR)
									, GETDATE()
									, @requestedByUserId);
		...
		INSERT INTO dbo.CourseNote(CourseId
							...
							VALUES (@toCourseId
									, 1 --General 
									, 'Client Name: ' + @fromClientName + ', Id: ' + CAST(@fromClientId AS VARCHAR) + ' - transferred into this course from course Id: ' + CAST(@fromCourseId  AS VARCHAR)
									, GETDATE()
									, @requestedByUserId);
