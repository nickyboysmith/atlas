/*
	SCRIPT: Amend SP uspSendNewPassword. Extra New Lines
	Author: Robert Newnham
	Created: 09/09/2016
*/
DECLARE @ScriptName VARCHAR(100) = 'SP026_14.04_Amend_uspSendNewPassword.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend SP uspSendNewPassword. Extra New Lines';
...
GO
	...
	*/
	...
	AS
	BEGIN
		IF NOT EXISTS (SELECT u.LoginId FROM [User] u WHERE LoginId = @LoginId)
		BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				INNER JOIN OrganisationSelfConfiguration OSC ON co.Id = OSC.OrganisationId
				WHERE u.LoginId = @LoginId
			END
			ELSE BEGIN
			...
			INNER JOIN OrganisationSystemConfiguration OSC ON ou.OrganisationID = osc.OrganisationId
			WHERE u.LoginId = @LoginId;
			...
			EXEC dbo.uspSetPassword @UserId = @User_Id, @Password = @RandomPassword
