...
DECLARE @ScriptName VARCHAR(100) = 'SP026_06.01_Amend_uspUpdateSystemInformation.sql';
...
AS
	...
	/* Checks to see if a parameter was passed through. If param passed through
	    then deletes relevant row. If no param passed through, deletes everything where SystemFeatureItemId
		is not null */

	IF (@systemFeatureItemId IS NULL)
		BEGIN
			DELETE FROM dbo.SystemInformation
			WHERE SystemFeatureItemId IS NOT NULL
		END
	ELSE
		BEGIN
			DELETE FROM dbo.SystemInformation
