/*
	SCRIPT: Add insert trigger to the UserFeedback table to send an email to the User and Atlas Administration
	Author: Nick Smith
	Created: 13/06/2016
*/

DECLARE @ScriptName VARCHAR(100) = 'SP021_51.01_AddInsertTriggerToUserFeedbackToSendEmailToUserAndAtlasAdministration.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Add insert trigger to the UserFeedback table to send an email to the User and Atlas Administration';
...
AS
		...
		SET @emailContent = 'This is an Automated reply.' + CHAR(13) + 'We have received your Feedback. Thank you.';

		IF (@responseRequired = 'Yes') 
			BEGIN

				SET @emailContent = @emailContent + 'We note that you have requested a follow up call or message. We will endeavour to get back to you as soon as possible.';
				SET @emailContent = @emailContent + CHAR(13) + 'Your Feedback was as follow:';
				SET @emailContent = @emailContent + CHAR(13) + 'Title: ' + @feedbackTitle + CHAR(13) + CHAR(13) + @feedbackBody;
			END
		...
		SET @emailContent = @emailContent + CHAR(13) + 'User''s URL: ' + @usersURL;
		SET @emailContent = @emailContent + CHAR(13) + 'User''s Email: ' + @usersEmail; 
		SET @emailContent = @emailContent + CHAR(13) + 'Respond To Email: ' + @atlasToEmailAddress; 
		SET @emailContent = @emailContent + CHAR(13) + 'Respond Required ' + @responseRequired;
		SET @emailContent = @emailContent + CHAR(13) + 'Feedback Created ' + @creationDate; 
		SET @emailContent = @emailContent + CHAR(13) + 'Title: ' + @feedbackTitle; 
		SET @emailContent = @emailContent + CHAR(13) + CHAR(13) + @feedbackBody; 
