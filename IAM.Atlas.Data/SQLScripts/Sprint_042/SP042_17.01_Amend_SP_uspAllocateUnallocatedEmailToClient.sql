...
DECLARE @ScriptName VARCHAR(100) = 'SP042_17.01_Amend_SP_uspAllocateUnallocatedEmailToClient.sql';
	...
	*/
	...
	BEGIN
		...
		BEGIN TRY 
			...
			SELECT DISTINCT TOP 100 --Incase there are loads only process 100 at a time
				CLE.ClientId			AS ClientId
				, SCET.ScheduledEmailId	AS ScheduledEmailId
			FROM dbo.ScheduledEmail SE
			INNER JOIN dbo.ScheduledEmailTo SCET		ON SCET.ScheduledEmailId = SE.Id
			INNER JOIN dbo.Email E						ON E.[Address] = SCET.Email
			INNER JOIN dbo.ClientEmail CLE				ON CLE.EmailId = E.Id
			LEFT JOIN dbo.ClientScheduledEmail CLSE		ON CLSE.ScheduledEmailId = SCET.ScheduledEmailId
			WHERE SE.DateCreated > DATEADD(DAY, -14, GETDATE()) --Only Look At Emails Created in the Last couple of Weeks
			AND LEN(ISNULL(SCET.ScheduledEmailId,'')) > 0
			AND CLSE.Id IS NULL;
		END TRY  
		BEGIN CATCH  
			SELECT 
