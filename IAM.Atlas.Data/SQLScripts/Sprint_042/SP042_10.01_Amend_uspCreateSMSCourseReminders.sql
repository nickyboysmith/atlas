/*
	SCRIPT: Amend uspCreateSMSCourseReminders
	Author: Dan Hough
	Created: 22/08/2017
*/

DECLARE @ScriptName VARCHAR(100) = 'SP042_10.01_Amend_uspCreateSMSCourseReminders.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Amend uspCreateSMSCourseReminders';
...
GO
	...
	*/
	...
	AS
	BEGIN
		DECLARE @ProcedureName VARCHAR(200) = 'uspCreateSMSCourseReminders'
			...
			*/
			...
			BEGIN		
				...
				BEGIN
					...
					BEGIN
						SET @ClientErrorMessage = @ClientPhoneNumberErrorMessage + CAST(@ClientId AS VARCHAR);
					END
				END
				ELSE
				BEGIN
					...
					EXEC @ScheduledSMSId =  dbo.uspSendSMS @RequestedByUserId
															, @ClientPhoneNumber
															, @Content
															, @ClientName
															, @ClientId
