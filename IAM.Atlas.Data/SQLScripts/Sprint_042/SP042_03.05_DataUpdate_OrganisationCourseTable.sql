...
DECLARE @ScriptName VARCHAR(100) = 'SP042_03.05_DataUpdate_OrganisationCourseTable.sql';
...
IF (dbo.udfAllScriptsCompletedExcept(@ScriptName) = 'True')
	BEGIN
		...
		SELECT DISTINCT CO.OrganisationId AS OrganisationId, CC.CourseId AS CourseId
		FROM dbo.CourseClient CC
		INNER JOIN dbo.ClientOrganisation CO ON CO.ClientId = CC.ClientId
		LEFT JOIN dbo.OrganisationCourse OC ON OC.OrganisationId = CO.OrganisationId
											AND OC.CourseId = CC.CourseId
