...
DECLARE @ScriptName VARCHAR(100) = 'SP043_01.01_Amend_SP_uspDatabaseTidyUpProcess.sql';
	...
	*/		
	...
	BEGIN
		...
		BEGIN TRY 	
			...
			/******************************************************************************************************/
			--Delete Old Trigger Log Information
			DECLARE @DeleteDatabaseTriggerLogsAfterDays INT = 35;
			DELETE DEL
			FROM [dbo].[TriggerLog] DEL
			WHERE DEL.[DateTimeRun] < DATEADD(DAY, (@DeleteDatabaseTriggerLogsAfterDays * -1), GETDATE());
			/******************************************************************************************************/
		END TRY  
		BEGIN CATCH  
			SELECT 
