...
DECLARE @ScriptName VARCHAR(100) = 'SP045_18.01_AlterSP_uspRefreshReconciliationData.sql';
DECLARE @ScriptComments VARCHAR(800) = 'Alter Stored procedure uspRefreshReconciliationData';
	...
	*/
	...
	BEGIN
		...
		BEGIN TRY
			...
			DECLARE @paymentStartDate DATE;
			DECLARE @paymentEndDate DATE;
			DECLARE @paymentTransactionDate DATE;
			...
			BEGIN
				...
				SELECT 
					@paymentId = p.Id,
					@paymentTransactionDate = convert(date, p.TransactionDate), 
				...
				INNER JOIN 
					...
					ON 
						...
						(convert(date, p.transactionDate) = rd.ReconciliationTransactionDate) AND
						(
							(
