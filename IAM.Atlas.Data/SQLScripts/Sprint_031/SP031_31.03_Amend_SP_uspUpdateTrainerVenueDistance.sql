...
DECLARE @ScriptName VARCHAR(100) = 'SP031_31.03_Amend_SP_uspUpdateTrainerVenueDistance.sql';
	...
	*/
	...
	BEGIN
		...
		BEGIN
			...
			WHERE UPPER(@venuePostCode) = REPLACE(postcode, ' ', '')
		END
		...
		BEGIN
			...
			FROM dbo.PostCodeInformation
			WHERE UPPER(@trainerPostCode) = REPLACE(postcode, ' ', '')
		END
