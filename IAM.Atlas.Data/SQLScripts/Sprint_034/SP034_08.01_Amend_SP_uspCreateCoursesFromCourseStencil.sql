...
DECLARE @ScriptName VARCHAR(100) = 'SP034_08.01_Amend_SP_uspCreateCoursesFromCourseStencil.sql';
	...
	*/
	...
	BEGIN 
		...
		SELECT	  @organisationId = ct.OrganisationId
				...
				, @reservedPlaces = cs.ReservedPlaces
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @venueCode = v.Code, @venueLocaleId = vl.Id
				FROM dbo.Venue v
				...
				IF(@venueCode IS NULL)
				BEGIN
					SET @venueCode = '';
				END

				PRINT 'Venue code: ' + @venueCode
